...
function collectGroups(grunt, config, opts) {
    ...
    /**
     * @param {string} taskOrTarget being tagged
     * @param {string|array} groupNames the task/target is tagged with
     ...
     * @returns {function}
     */
    function createTask(grunt, name, subTasks) {

        return function () {
            grunt.log.ok('Queing tasks: ' + grunt.log.wordlist(groupTasks[name]));
            _.forEach(subTasks, function (task) {
                grunt.task.run(task);
            });
        };
    }
    ...
    for (group in groupTasks) {
        grunt.registerTask(group, createTask(grunt, group, groupTasks[group]));
        if (grunt.option('verbose')) {
    ...
    _.forEach(groups, function (group) {
        if (!grunt.task.exists(group)) {
            ...
            grunt.registerTask(group, function () {
                grunt.log.warn('empty... add tasks by tagging them with "' + opts.tag + ': ' + group + '".');
            });
        }
    });
}
